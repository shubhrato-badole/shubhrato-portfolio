    ...
    <section id="skills" className="max-w-6xl mx-auto px-6 py-24">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        ...
        <div className="flex items-center gap-3 overflow-x-auto pb-2 md:pb-0 md:flex-wrap -mx-6 px-6 md:mx-0 md:px-0">
          {(['all', 'frontend', 'backend', 'ai', 'tools'] as Category[]).map((cat) => (
            <button
              key={cat}
              onClick={() => { setCategory(cat); setActiveSkill(null); }}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {filteredSkills.map((skill) => {
          const isOpen = hoveredSkill === skill.name || activeSkill === skill.name
          return (
            <div key={skill.name}
              ...
              style={{
                background: isOpen ? `${skill.color}18` : 'rgba(255,255,255,0.03)',
                border: `1px solid ${isOpen ? skill.color + '55' : 'rgba(255,255,255,0.07)'}`,
                transform: isOpen ? 'translateY(-3px) scale(1.04)' : 'none',
                boxShadow: isOpen ? `0 0 30px ${skill.color}22` : 'none',
              }}>
              <span className="text-2xl">{skill.icon}</span>
              <span className="font-mono text-xs text-slate-400">{skill.name}</span>

              {isOpen && (
                <div className="absolute top-full mt-3 left-1/2 w-44 rounded-xl p-3 z-20"
                  style={{
                  ...
                  <div
                    ...
                    style={{ background: '#0a0118', border: `1px solid ${skill.color}40`, borderRight: 'none', borderBottom: 'none' }}
                  />
                </div>
              )}
            </div>
